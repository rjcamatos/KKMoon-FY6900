...
void WaveFormEditor::paintEvent(QPaintEvent *event)
{
   ...
   if( hoverPoint > -1 ) {
        ...
        while( (hoverPoint+rc) > ((int)markPointsSize-1) ) rc--;
...
void WaveFormEditor::drawFree(QMouseEvent *event, int contiguousLeft, int contiguousRight)
{
    ...
    for( uint n = 0; n < linePointsSize; n++ ) {
        if( linePoints[n].x() > (posX - sX(2)) && linePoints[n].x() < (posX + sX(2))  ) {
            ...
            markPoints[n].setRect(linePoints[n].x(),posY-tY(0.2),tX(0.4),tY(0.4));
        }
    }
...
void WaveFormEditor::drawRectangle(QMouseEvent *event, int contiguousLeft, int contiguousRight)
{
    ...
    for( int n = 0; n < contiguousLeft; n++ ) {
        ...
        markPoints[(selectedPoint-n)].setRect(linePoints[selectedPoint-n].x(),posY-tY(0.2),tX(0.4),tY(0.4));
    }
...
void WaveFormEditor::drawTriangle(QMouseEvent *event, int contiguousLeft, int contiguousRight)
{
    ...
    for( int n = 0; n < contiguousLeft; n++ ) {
        ...
        markPoints[selectedPoint-n].setRect(linePoints[selectedPoint-n].x(),lHeight-tY(0.2),tX(0.4),tY(0.4));
        lHeight -= lIncrement;
    }
    ...
    for( int n = 0; n < contiguousRight; n++ ) {
        ...
        markPoints[selectedPoint+n].setRect(linePoints[selectedPoint+n].x(),rHeight-tY(0.2),tX(0.4),tY(0.4));
        rHeight -= rIncrement;
    }
...
void WaveFormEditor::drawArch(QMouseEvent *event, int contiguousLeft, int contiguousRight)
{
    ...
    for( int n = 0; n < contiguousLeft; n++ ) {
        ...
        markPoints[selectedPoint-n].setRect(linePoints[selectedPoint-n].x(),linePoints[selectedPoint-n].y()-tY(0.2),tX(0.4),tY(0.4));
...
void WaveFormEditor::drawInflexedArch(QMouseEvent *event, int contiguousLeft, int contiguousRight)
{
    ...
    for( int n = 0; n < contiguousLeft; n++ ) {
        ...
        markPoints[selectedPoint-lCount].setRect(linePoints[selectedPoint-lCount].x(),linePoints[selectedPoint-lCount].y()-tY(0.2),tX(0.4),tY(0.4));
    ...
    for( int n = 0; n < contiguousRight; n++ ) {
        ...
        markPoints[selectedPoint+rCount].setRect(linePoints[selectedPoint+rCount].x(),linePoints[selectedPoint+rCount].y()-tY(0.2),tX(0.4),tY(0.4));
...
void WaveFormEditor::resetLinesAndMarks()
{
    ...
    posX = 0;
    for(uint n = 0; n < markPointsSize; n++ ) {
        markPoints[n].setRect(posX-tX(0.2),posY-tY(0.2),tX(0.4),tY(0.4));
        posX += increment;
    }
...
void WaveFormEditor::onChannelOrSlotChange()
{
    ...
    if( file.open(QIODevice::ReadOnly) ) {
        for(uint n = 0; n < linePointsSize; n++) {
            ...
            markPoints[n].setRect(n*((qreal)windowSizeX/(qreal)linePointsSize),value-tY(0.2),tX(0.4),tY(0.4));
        }
        file.close();
        startEdit = true;
    }
